...
// Página de ingresos (movimientos filtrados)
...
export default function IngresosPage() {
  ...
  // Filtro base: SIEMPRE ingresos
  const [filters, setFilters] = useState<MovementFilters>({ month: selectedMonth, type: 'ingreso' })
    ...
    <div className="flex flex-col gap-6 fade-in max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-positive-light">
            <TrendingUp size={20} className="text-positive" />
          </div>
          <div>
            <h1 className="text-xl font-bold" style={{ color: 'var(--foreground)' }}>Ingresos</h1>
            <p className="text-sm" style={{ color: 'var(--foreground-muted)' }}>
              Historial de tus ingresos mensuales
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleOpenCreate} className="btn desktop-only" style={{ background: 'var(--positive)', color: 'white' }}>
            <Plus size={18} />
            Nuevo Ingreso
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            <div className="mobile-only flex flex-col gap-3 pb-16">
              {movements?.length === 0 ? (
                <div className="card p-8 text-center text-foreground-muted">No hay ingresos registrados.</div>
              ) : (
                movements?.map(mov => (
      ...
      <ConfirmDialog 
        ...
        description={`¿Eliminar el ingreso "${movementToDelete?.description}" por ${formatCurrency(movementToDelete?.amount || 0)}?`}
        dangerous
